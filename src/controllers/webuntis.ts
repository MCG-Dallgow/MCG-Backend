...
export async function getStudentGroup(untis: WebUntis): Promise<string> {
...
export async function startWebUntisSession(username: string, webuntisKey: string): Promise<WebUntis | undefined> {
...
export async function authenticate(req: Request, res: Response, requireUser: boolean): Promise<[WebUntis | undefined, User | undefined]> {
    ...
    // check WebUntis credentials and start API session
    var untis = await startWebUntisSession(username, password);
    if (!untis) {
        return [undefined, undefined];
    }

    // check if user is in database
    var user: User = (await db.select().from(users).where(eq(users.username, username)))[0];
...
export const login: RequestHandler = async (req, res) => {
    ...
    if (!user) {
        ...
        user = (await db.select().from(users).where(eq(users.username, student.name)))[0];
    }
